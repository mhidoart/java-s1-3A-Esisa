...
public class DocumentDaoJdbc implements AuthorsDao {
	private Database db;
	...
	public List<Authors> selectAll() {
		...
		if (data == null) return null;
		Vector<Authors> docs = new Vector<>();
		for (int i = 1; i < data.length; i++) {
			Authors auth = BiblioMapper.getDocument(data[i]);
			auths.add(auth);
		}
		return docs;
	}
	...
	public List<Document> selectByKeyword(String keyword) {
		...
		if (data == null) return null;
		Vector<Authors> auths = new Vector<>();
		for (int i = 1; i < data.length; i++) {
			Authors auth = BiblioMapper.getDocument(data[i]);
			auths.add(auth);
		}
